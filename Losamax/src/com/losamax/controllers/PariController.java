...
//		 eventRepo.save(e8);
		...
		if (AuthHelper.isAuthenticated()) {
			if (AuthHelper.getRole().equals(ERole.ROLE_USER)) {
				...
				Client client1 = clientRepo.findByCredentialsUsername(credentials.getUsername());
				model.addAttribute("client", client1);
			} else if (AuthHelper.getRole().equals(ERole.ROLE_ADMIN)) {
				Credentials credentials = AuthHelper.getCredentials();
				Admin admin1 = adminRepo.findByCredentialsUsername(credentials.getUsername());
		...
		List<Evenement> levent = eventRepo.findByEvenementFutur(d1);
	...
	public String gotorubrique(@PathVariable(value = "nom") String nom, Model model) {
		...
		if (AuthHelper.isAuthenticated()) {
			if (AuthHelper.getRole().equals(ERole.ROLE_USER)) {
				Credentials credentials = AuthHelper.getCredentials();
				Client client = clientRepo.findByCredentialsUsername(credentials.getUsername());
				model.addAttribute("client", client);
			} else if (AuthHelper.getRole().equals(ERole.ROLE_ADMIN)) {
				Credentials credentials = AuthHelper.getCredentials();
				Admin admin = adminRepo.findByCredentialsUsername(credentials.getUsername());
	...
	public String compte(@PathVariable(value = "id") Long id, @PathVariable(value = "username") String username,
			Model model) {
